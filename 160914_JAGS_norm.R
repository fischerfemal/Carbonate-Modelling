...
bayes_model = " model {
  ...
  for (i in 1:BB_Bkg) {
    ...
    dO_P_Bkg_BB[i] <- -13.6 + dO_MAT_sl_sp * MAT_Bkg_BB
  ...
  for (j in 1:BB_PETM) {
    ...
    dO_P_PETM_BB[j] <- -13.6 + dO_MAT_sl_sp * MAT_Bkg_BB + dO_MAT_sl_tm * (ch_MAT + ch_ch_MAT_BB)
  ...
  for (k in 1:AC_Bkg) {
    ...
    dO_P_Bkg_AC[k] <- -13.6 + dO_MAT_sl_sp * MAT_Bkg_AC
  ...
  for (l in 1:AC_PETM) {
    ...
    dO_P_PETM_AC[l] <- -13.6 + dO_MAT_sl_sp * MAT_Bkg_AC + dO_MAT_sl_tm * (ch_MAT + ch_ch_MAT_AC)
  ...
  for (e in 1:ES_Bkg) {
    ...
    dO_P_Bkg_ES[e] <- -13.6 + dO_MAT_sl_sp * MAT_Bkg_ES
  ...
  for (f in 1:ES_PETM) {
    ...
    dO_P_PETM_ES[f] <- -13.6 + dO_MAT_sl_sp * MAT_Bkg_ES + dO_MAT_sl_tm * (ch_MAT + ch_ch_MAT_ES)
  ...
  for (g in 1:CN_Bkg) {
    ...
    dO_P_Bkg_CN[g] <- -13.6 + dO_MAT_sl_sp * MAT_Bkg_CN
  ...
  for (o in 1:CN_PETM) {
    ...
    dO_P_PETM_CN[o] <- -13.6 + dO_MAT_sl_sp * MAT_Bkg_CN + dO_MAT_sl_tm * (ch_MAT + ch_ch_MAT_CN)
  ...
  pCO2_Bkg ~ dnorm(500, 0.0001)I(200, 2000)
  deltaA_Bkg ~ dnorm(-5, 11)I(-6, -4)
  dO_MAT_sl_sp ~ dnorm(0.5, 100)I(0.2, 0.7)
  dO_MAT_sl_tm ~ dnorm(0.3, 100)I(0.1, 0.5)
  
  MAP_Bkg_BB ~ dnorm(1000, 0.000025)I(100, 2000)
  MAP_Bkg_AC ~ dnorm(600, 0.000011)I(100, 2000)
  ...
  ch_ch_MAT_BB ~ dnorm(0, 1.78)I(-2, 2)
  ch_ch_MAT_AC ~ dnorm(0, 1.78)I(-2, 2)
  ch_ch_MAT_ES ~ dnorm(0, 1.78)I(-2, 2)
  ch_ch_MAT_CN ~ dnorm(0, 1.78)I(-2, 2)
  
  MAP_PETM_BB ~ dnorm(1000, 0.000011)I(100, 2000)
  MAP_PETM_AC ~ dnorm(600, 0.000011)I(100, 2000)
  MAP_PETM_ES ~ dnorm(1000, 0.00000625)I(100, 2000)
  MAP_PETM_CN ~ dnorm(1000, 0.00000625)I(100, 2000)
  
  P_seas_Bkg_BB ~ dnorm(0.15, 100)I(0, 0.4)
  P_seas_Bkg_AC ~ dnorm(0.15, 100)I(0, 0.4)
  P_seas_Bkg_ES ~ dnorm(0.15, 100)I(0, 0.4)
  P_seas_Bkg_CN ~ dnorm(0.15, 100)I(0, 0.4)

  P_seas_PETM_BB ~ dnorm(0.15, 100)I(0, 0.4)
  P_seas_PETM_AC ~ dnorm(0.15, 100)I(0, 0.4)
  P_seas_PETM_ES ~ dnorm(0.15, 100)I(0, 0.4)
  P_seas_PETM_CN ~ dnorm(0.15, 100)I(0, 0.4)
  ...
  ST_Bkg_BB ~ dnorm(0.5, 11)I(0, 1)
  ST_Bkg_AC ~ dnorm(0.5, 11)I(0, 1)
  ST_Bkg_ES ~ dnorm(0.5, 11)I(0, 1)
  ST_Bkg_CN ~ dnorm(0.5, 11)I(0, 1)

  ST_PETM_BB ~ dnorm(0.5, 11)I(0, 1)
  ST_PETM_AC ~ dnorm(0.5, 11)I(0, 1)
  ST_PETM_ES ~ dnorm(0.5, 11)I(0, 1)
  ST_PETM_CN ~ dnorm(0.5, 11)I(0, 1)
...
bayes_fit <- jags(model.file = textConnection(bayes_model), parameters.to.save = parameters, data = carbonate_data, inits = NULL, 
                  n.chains=3, n.iter = 10000000, n.burnin = 2000000, n.thin = 1000
)



bayes_fit_mcmc <- as.mcmc(bayes_fit)
pdf("160921_jags_upd_plots.pdf")
...
denplot(bayes_fit_mcmc, "deltaA_Bkg")
...
save.image("160921_JAGS_Output.RData")


# update if there is little convergence

bayes_fit_upd <- autojags(bayes_fit, n.iter=8000000, n.thin=1000, n.update = 3)
